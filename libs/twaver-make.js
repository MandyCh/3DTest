(function(window, undefined) {
     ...
     * 路径连线
     */
    var create3DPath = function(pathData) {
        var path;
    ...
    make.Default.register('twaver.xianlu.connection', function(json) {
        ...
        var connection = new mono.PathNode(path, 100, 2);
        connection.s({
            'm.type': 'phong',
    ...
    make.Default.register('twaver.idc.smoke', function(json) {
        var position = json.position || [0, 0, 0];
        var color = json.color;
        var x = position[0],
            y = position[1],
        ...
        smoke.s({
            ...
            'm.transparent': true,
            'm.opacity': 0.5,
            'm.texture.image': '../res/images/smoking.png',
            'm.color': color,
            'm.depthTest': false
        })
    ...
    //           A/___|
    //              x
    make.Default.register('twaver.idc.Infrared', function(json) {
        var angle = Math.atan2(json.to[1] - json.from[1], json.to[0] - json.from[0]); //返回X轴到点（x，y）之间的角度，atan2（斜率）获得一个角度，y坐标在前（y,x），返回值是反正切值
        var offset = 1.5; //
        var fromOffsetZ = offset * Math.sin(angle); //正弦
        var fromOffsetX = offset * Math.cos(angle); //余弦
        var toOffsetZ = offset * Math.sin(angle + Math.PI);
        var toOffsetX = offset * Math.cos(angle + Math.PI);

        var envmap = [
                '../res/images/rack_inside.jpg',
                '../res/images/rack_inside.jpg',
                '../res/images/rack_inside.jpg',
                '../res/images/rack_inside.jpg',
                '../res/images/rack_inside.jpg',
        ...
        pole.s({
            'm.texture.image': '../res/images/rack_inside.jpg',
            'm.color': '#A4A4A4',
            'm.ambient': '#A4A4A4',
        ...
        pole2.setPosition(json.to[0] - json.from[0], pole2.getHeight() / 2 - 80, json.to[1] - json.from[1]);
        pole2.setClient('type', 'laser');
        pole2.setParent(pole); //绑定在pole上面输出
        ...
        for (var i = 0; i < 5; i++) {
            ...
            from.setPosition(0, -50 + i * 27, 0);
            from.setClient('type', 'laser');
            from.setParent(pole);
            from.setVisible(false);

            var to = from.clone();
            to.setPosition(json.to[0] - json.from[0], -50 + i * 27, json.to[1] - json.from[1]);
            to.setParent(pole);
            to.setClient('type', 'laser');
            to.setVisible(false);

            link = new mono.Link(from, to);
            link.s({
                'm.color': color,
                'm.ambient': color,
                'm.transparent': true,
    ...
    var registerEquipment = function(size) {
        make.Default.register('twaver.idc.equipment{}'.format(size), function(json, callback) {
            ...
            make.Default.copyProperties(json, newData);
            var server = make.Default.load(newData, callback);
            server.setClient('type', 'equ{}'.format(size));
            return server;
    ...
    function showVideoDialog(title) {
        var video = decument.createElement('video');
